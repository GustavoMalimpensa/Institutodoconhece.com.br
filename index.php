	...
	<section class="areaatuacao">
		...
		<div class="container">
			...
			<div class="row sliderdefault sliderareaatuacao owl-carousel owl-theme">
				...
				<?php 
				    $q = "SELECT *, c.titulo as tituloCategoria, c.imagem as imagem,  t.id as id, c.titulo as titulo, t.texto as texto FROM categorias c INNER JOIN treinamentos t ON c.id = t.categoria GROUP BY t.categoria ORDER BY c.id = 13 DESC, c.id = 10 DESC, c.id = 11 DESC, c.id = 14 DESC";
				    $q = mysql_query($q);
				    while ($r = mysql_fetch_array($q)) {
					    $id = $r["id"];
					    $titulo = $r["titulo"];
